...
function get_query_result(which_btn) {
    ...
    $.getJSON(full_url, function (data) {
        ...
        //logging all owners that satisfied query (unnecessarily large query, but useful for debugging)
        while (i < data.features.length) {
            console.log("Owner: " + data.features[i].attributes.OWNER_NAME);
            console.log("Full address: " + data.features[i].attributes.ANUMBER + " " + data.features[i].attributes.ST_NAME + " " + data.features[i].attributes.ST_POS_TYP + " " + data.features[i].attributes.ST_POS_DIR + " " + data.features[i].attributes.CTU_NAME + ', ' + data.features[i].attributes.STATE_CODE + " " + data.features[i].attributes.ZIP);
            console.log("County name: " + data.features[i].attributes.CO_NAME);
            console.log("Estimated property value (in USD): " + data.features[i].attributes.SALE_VALUE);
            i++;
        }
        i = 0;
